...
SELECT COUNT(*) as total_num FROM customers;
...
SELECT COUNT(*) AS ca_order_num FROM orders WHERE shipto_state='CA';
...
SELECT SUM(order_total) AS money_total FROM orders;
...
SELECT AVG(order_total) as ave_cost FROM orders;
...
SELECT MIN(order_total) as min_cost FROM orders;
...
SELECT SUM(order_total) AS total_internet_orders FROM orders WHERE salesperson_id IS NULL;